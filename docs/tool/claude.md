...
!!! tip "搞不定科学上网的同学，换成看另外一篇"
    ...
    **整篇教程的前置条件是科学上网**，没弄过代理的同学光这一步就可能卡很久。
    ...
    这种情况可以直接看另一篇：DeepSeek 工具链配置。那篇装的是社区封装的 **DeepSeek Harness 桌面版**，下载、安装、调用模型全程都在国内完成，**不需要任何代理**，装完把 API Key 填进去就能用，少了一大堆配置步骤。唯一的代价是它还是个较新的项目，偶尔会踩到 bug。
...
!!! tip "`code` 提示找不到命令"
    这个命令要装了 VSCode **并且连上 WSL** 之后才生效（见 WSL2 环境搭建那篇的第 6 步）。没装上就直接用记事本那条 —— WSL 能直接调用 Windows 的程序，记事本打开的确实就是 WSL 里的那份 `~/.bashrc`。
...
前面第 8 步装的是**终端里**的 Claude Code。它还有一个 **VSCode 扩展**，装完之后不用切窗口 —— Claude Code 就在编辑器里，**选中的代码会自动进上下文**，改了什么也直接在编辑器里看得见。

!!! warning "顺序不能反：先装 Claude Code，再装扩展"

    扩展**不自带** Claude Code。它只是在编辑器里给 Claude Code 套了一层界面，真正干活的还是第 8 步装的那个 `claude` 命令。
...
!!! warning "用 WSL 的同学：两侧都要装"

    截图里那行 `Extension is enabled on 'SSH: mind-city'`，意思是「已经在这个远程环境里启用了」。

    WSL 同学的情况是一样的：**Windows 那一侧和 WSL 那一侧各装一份**。在扩展面板里点 **「Install in WSL: Ubuntu」** 那个按钮就行 —— 和 WSL2 环境搭建那篇里装 C/C++ 扩展时是同一个按钮。

### 9.2 从内置终端里启动

**面板不会自己冒出来**，要从 VSCode **自带的内置终端**里把 Claude Code 跑起来：

1. 用 VSCode 打开你写代码的那个文件夹
2. 点顶部菜单的 **「终端」→「新建终端」**（英文界面是 Terminal → New Terminal）
3. 在这个终端里敲 `cc` 回车（就是 8.3 节里那个快捷方式）

跑起来之后，编辑器里就会出现 Claude Code 的面板，和「终端」并排（下一节的截图里能看到）。
...
- **改动怎么给你看**：扩展知道你正开着哪个文件、选中了哪段，改动会直接提在编辑器窗口里；终端版则是在对话里把 diff 贴出来
- **跑长任务、批量处理**：用终端更顺手

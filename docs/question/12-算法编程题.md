...
### 5 · 数组双指针划分（快排 partition · 程序补完） [高频]
...
??? note "解析"
    ...
    **四个关键设计**：
    ...
    !!! warning "题目代码的一处笔误"
        结构体定义里写的是 `struct node * next;`（小写 `node`，没有这样的类型），应为 `struct intNode * next;`。
...
!!! warning "题干缺失"
    本题只保留参考实现，题面文字没有留存，下面是由参考代码反推的题意（**不是题干原文，不要当题干背**）。
...
!!! warning "题干缺失"
    本题只保留了**答案**（`PS: (2)和(3)顺序可换` + 6 个空的答案），题目代码和题干都没有留存，**无法反推出完整的题目代码**。从答案可反推是在求**二维前缀和**（`prefixSum[i][j]` 表示左上角 `(0,0)` 到 `(i,j)` 的子矩阵元素之和）。
...
!!! warning "函数名以框架代码为准"
    题面第 3 条写的是实现 `sumpoly` 与 **`output`**，但框架代码里 `main` 调用的是 **`outprint`**（参考答案也是 `outprint`）。两者必有一处笔误——**照着框架代码写 `outprint`**，写成 `output` 会因找不到该函数而**链接失败**。

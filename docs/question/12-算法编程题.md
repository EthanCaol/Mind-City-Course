...
> 本文件按 9 个「【模板】通用套路 + 对应真题」组织，把每个模板的套路并入该题解析的开头一段，
> 其余的仍按「原题 / 答案 / 解析」排。
...
> 下文「答案」里的代码一律按参考答案原样收录（不重新推导、不"顺手改进"），可直接填回题目框架编译运行。
...
??? note "解析"
    ...
    > 为什么从后往前？ 从前往后会覆盖掉还没搬走的数据（和 07 节 `ostr` 一个道理）。
    ...
    > 所以注释（"数组已满""元素后移"）、缩进、变量命名都是要计分的——本参考答案的注释就是标准范式。
...
> 同一道题也收在 `08-结构体与链表.md` 第 12 题（那边从「链表数据结构」的角度讲）。

已知一个不带辅助表元的单向链表，每个结点保存一个整数值。要求完善函数实现删除链表中所有值为奇数的结点，返回最终链表的头指针。（12 分）
...
??? note "解析"
    ...
    4. 相等时取 `h1`（`<=`）保证稳定性（相等元素保持"h1 在前"的原有相对顺序）。注意：这条只对归并成立，插入排序里不是这么回事（见题 7）。
...
> 同一道题也收在 `08-结构体与链表.md` 第 11 题（那边从「链表数据结构」的角度讲）。
...
??? note "解析"
    ...
    - ②内层循环 `while(p2 && p2->data < p->data)`：找第一个不小于 `p->data` 的结点 `p2`，插入点就在它前面（由 `u2` 记录前驱）。
    - ③`if (p2 == h2)`：`p2` 没动过，说明要插到最前面——这时必须更新头指针 `h2`，这是最容易漏的一步。
    - ④ `u2->next = p; p->next = p2;`：标准的"中间插入"两句话。

    > 关于稳定性（重要，别记错）：内层用的是 `<`、前插又是无条件插在 `h2` 最前面，所以后取出的相等元素会被插到先取出的相等元素前面——本函数不保稳定。
...
??? note "解析"
    ...
    - `#define max(a,b) a>b?a:b` —— 这里恰好没出问题，但这个宏有展开陷阱（见 11 节），工程上应写 `#define max(a,b) ((a)>(b)?(a):(b))`。
...
!!! warning "题干缺失"
    本题只保留了答案（`PS: (2)和(3)顺序可换` + 6 个空的答案），题目代码和题干都没有留存，无法反推出完整的题目代码。从答案可反推是在求二维前缀和（`prefixSum[i][j]` 表示左上角 `(0,0)` 到 `(i,j)` 的子矩阵元素之和）。
...
??? note "解析"
    ...
    - 降序口诀：`if (a[j] < a[j+1])` 就交换 → 小的往后沉 → 结果是降序。（升序是 `a[j] > a[j+1]` 交换，别记反。）
    - `fac` 的两要素：
...
!!! warning "函数名以框架代码为准"
    题面第 3 条写的是实现 `sumpoly` 与 `output`，但框架代码里 `main` 调用的是 `outprint`（参考答案也是 `outprint`）。两者必有一处笔误——照着框架代码写 `outprint`，写成 `output` 会因找不到该函数而链接失败。
...
??? note "解析"
    ...
    ① 归并式双指针 —— 这是必考模板，和"两个有序链表合并"（题 6）完全同构：
...
??? note "解析"
    ...
    坑 1：累加项必须是浮点。 如果写成 `e += 1 / fact;`（两个 int 相除），`1/2` 在 C 里是整数除法，结果直接是 0，`i >= 2` 之后全加 0，最后只会输出 1（或 2），这是本题最容易丢分的地方。只要除法两边有一个是 `double`，整个除法就是浮点除法——所以要么写 `1.0 / fact`，要么像我上面的递推写法那样让 `term` 本身是 `double`。
...
2. 题 6（归并两个有序链表）的"带不带辅助表元"题目没有明说：题目只给了 `struct intNode` 定义和函数声明，没说链表有没有辅助表元。正文两版实现都给出（带辅助表元 / 不带辅助表元），并注明"假设带辅助表元——与其他题一致"。答题时按其他题的约定判断。
3. 题 7 代码里的类型名笔误：结构体成员写成 `struct node * next;`（应为 `struct intNode * next;`），gcc 15.2.0 实测报 `incompatible pointer type` 硬错误，编译不过。正文的代码块原样保留、未改动，只在解析里加了说明；若要真正跑通，把该成员改成 `struct intNode * next;` 即可（改后实测输出与答案一致）。
4. 三处题干缺失（题 9 矩阵范数、题 10 合并两个有序数组、题 11 二维前缀和）：这三题都只保留了参考答案。题 9、题 10 的题意是从参考答案代码反推的（不是题干原文），题 11 连代码都没有留存，只有 6 个空的答案，因此只能给出答案表 + 前缀和递推公式。三题均已按"题干缺失，由参考答案反推"标注，未编造题干。

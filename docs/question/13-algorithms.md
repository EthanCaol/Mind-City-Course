...
> 命题规律：历年编程题从不要求从零写完整程序，一律是"给了 `main` 和文件头，只补函数体"。
> 考点高度集中在四类：① 数组原地操作 ② 链表增删改查 ③ 字符串处理 ④ 二分 / 排序。
>
> 本文件按 7 个「【模板】通用套路 + 对应真题」组织，把每个模板的套路并入该题解析的开头一段，
> 其余的仍按「原题 / 答案 / 解析」排。
>
> 评分权重：评分标准明确"90% 为程序正确性评分，10% 为程序其他质量指标评分（可读性、书写风格等）"——
> 注释、缩进、变量命名都是要计分的，编程题务必写得干净。

> 模板速查：① 数组原地插入（题 1）｜② 链表冒泡（题 2、10）｜③ 二分查找（题 3）｜④ 双指针划分 / 快排 partition（题 4）｜
> ⑤ 归并（题 5、8、11）｜⑥ 综合算法（题 6）｜⑦ 其他真题速览（题 7、9）。
>
...
??? note "解析"
    ...
    > 所以注释（"数组已满""元素后移"）、缩进、变量命名都是要计分的——材料上这份答案的注释就是标准范式。
...
??? note "解析"
    ...
    > ② `sort` 和 `fac` 在 `main` 之前没有声明/定义 → gcc 14 起这是硬错误（`error: implicit declaration of function`），不是警告，`-w` 也压不住（gcc 15.2.0 实测：编译直接失败）。

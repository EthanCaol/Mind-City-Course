...
??? note "解析"
    ...
    - 另外第 13 行少了一句 `free(...)`（释放被删结点），但"不得增行"所以不好补——这也是题目本身设计得不够严谨的地方。
...
??? note "解析"
    ...
    - 第 7 行的正确写法是 `temp->next != NULL`——让 `temp` 停在最后一个结点上，这样才能在第 9 行处接上新结点。
      - 对比记忆：要"停在尾结点"，条件写 `temp->next != NULL`；要"走完全程"，条件写 `temp != NULL`。（第 5 题第 12 行的 `p->next->data`、第 8 题 25.7 的 `node->data <= min` 用的是同一条规律。）
    ...
    !!! warning "改完这 3 处，空链表上仍会崩"
        但这 3 处都改完，程序在「空链表」上仍会崩（真机实测：`Segmentation fault`，退出码 139）。原因：第 4 行 `temp = head;` 写在 `if (head == NULL)` 之前，所以空表时 `temp` 恒为 `NULL`，而第 9 行的 `temp->next = newNode;` 要求 `temp` 非空 → 解引用空指针。
...
??? note "解析"
    ...
    !!! quote "同一个原则的正反两面"
        ...
        和第 6 题对照：第 6 行的 `insertEnd` 出错在"改的是 `temp` 而不是 `temp->next`"；本题是"改的是 `current->next` 而不是 `current`"——同一个原则的正反两面。
...
??? note "解析"
    ...
    4. 内层 `while` 找的是 `p->next`，这样 `p` 停在"插入位置的前驱"上。（同第 8 题的 4.2.1、第 5 题第 12 行。）

...
!!! failure "如果提示 `wsl` 不是内部或外部命令"
    说明 2.2 步的安装包没装上，回去重装一次再试。

    如果重装了还是不行，检查 2.1 步的两个 Windows 功能有没有勾上。

    还是不行的话，请马上向附近同学或者助教求助。
...
Ubuntu 的软件都要从网上下载，所以这一步先把网络理顺，再装编译工具。

- 如果你**不**熟悉科学上网 → 走 [4.1 切换镜像源](#41-切换镜像源并更新可选)

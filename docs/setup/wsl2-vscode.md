...
先弄清楚 Default 是什么，后面才不容易绕：VSCode 把你现在这套配置本身叫做 Default（默认配置文件）—— 你从装好 VSCode 到现在改过的设置、装过的插件，全都记在它名下。它不是另一套环境，而是你此刻正在用的这套。

一个配置文件和 Default 之间是什么关系，是在新建时的那张表单里逐项决定的。设置、快捷键、任务、代码片段、插件、MCP 服务器这六类内容，每一类都能单独选「从哪来」：

- 选 Default：这一类内容由 Default 提供 —— 你在 Default 里装了什么、改了什么，这个配置文件里就是什么，两边是同一份。
- 选 Never：这一类内容归这个配置文件自己管，Default 那边什么样都跟它无关。

所以「新建一个配置文件」并不是凭空多出一套环境，而是把其中几类内容划出来交给它自己管。这门课要的是一套独立、干净的环境，就让六类内容全部自己管 —— 表单里 Copy from 选 None 就是这个效果，右边 Contents 那一栏会跟着全部变成 Never。

之后视频里装什么插件、改什么设置，都只落在这个配置文件里，你平时用的 Default 一点不受影响。哪天不想要了，在列表里把它删掉，回到 Default 就行。

#### 第 1 步：打开配置文件页面

点左下角的齿轮图标（Manage / 管理），菜单里有一项 Profiles（如果你已经建过配置文件，这一项会显示成 `Profile (配置文件名字)`）。鼠标移上去会展开子菜单，在子菜单里点 Profiles：

![管理菜单里的 Profiles 子菜单](https://image-1379176255.cos.ap-shanghai.myqcloud.com/20260921113940672.png)

#### 第 2 步：新建并保存

配置文件页面打开后，点左上角的 New Profile，右边会出现一张表单：

- Name：填名字，比如 `Mind-City`
- Copy from：保持 None

填完点下面的 Create 保存：

![New Profile 表单中填好名字后点 Create](https://image-1379176255.cos.ap-shanghai.myqcloud.com/20260921113954720.png)

!!! tip "六类内容不用一起选"
    比如你希望插件独立、快捷键沿用顺手的那套：Copy from 选 None，再把 Keyboard Shortcuts 那一行点成 Default 就行 —— 这个配置文件不自己管快捷键，激活它的时候用的还是 Default 里的那份。
...
启用之后，它旁边会显示 Active 字样（同一个位置原来是 Default 或上一套配置在显示）。当前正在用哪个配置文件，在窗口标题栏、以及鼠标悬停在左下角齿轮上时也能看到。

配置文件还是跟着文件夹走的：新建或启用一个配置文件时，当前打开的文件夹就和它绑上了，以后再打开这个文件夹会自动切回来。你可以在配置文件的 Folders & Workspaces 一栏里看它绑定了哪些文件夹。

到这里准备工作就完成了，视频后面的内容可以接着往下做。以后想切回原来的配置，在同一个列表里勾选 Default 就行。

### 7.2 把配置同步到云端（Settings Sync）

配置文件只存在这台电脑上。以后要是换了电脑、或者宿舍一台实验室一台，用 VSCode 自带的「设置同步」（Settings Sync）就能把这套配置存到云端，在另一台机器上登录同一个账号，配置自动恢复过来。登录用 GitHub 账号（或微软账号）都可以。

#### 第 1 步：打开同步设置

点左下角的齿轮图标（Manage / 管理），选 Backup and Sync Settings...。活动栏最下面那个账户图标里也有同一个入口。

如果菜单里显示的是 Settings Sync is On，说明同步已经开着，点它进去就能改同步哪些内容。

#### 第 2 步：登录 GitHub

点 Sign in，在弹出的选项里选 GitHub，浏览器会打开让你登录并授权。授权完成后浏览器那边直接关掉，VSCode 会自己接上。

!!! note "GitHub Enterprise 账号不支持"
    用普通的个人 GitHub 账号登录就行。

#### 第 3 步：勾选要同步的内容

登录之后会让你选同步哪些内容，把下面这些都勾上：

- Settings：所有设置项
- Keyboard Shortcuts：快捷键
- User Snippets：代码片段
- User Tasks：任务
- UI State：界面状态（显示语言、活动栏和面板上的图标、视图布局等）
- Extensions：装过的插件
- Profiles：配置文件本身

Profiles 这一项单独说一下：勾上它，你建的这几套配置文件在同步过的机器上都可用，新建的配置文件、以及每个配置文件里装了哪些插件，都会一并带过去。

!!! warning "WSL 那一端的插件同步不过去"
    VSCode 官方明确写了：不会把扩展同步到远程窗口，WSL、SSH、开发容器都不在同步范围内。所以 [6.4](#64-把插件再在-wsl-里装一遍) 在 WSL 里装的 C/C++ 和 Code Runner，到了新电脑上还得在 WSL 里再装一遍。这里能同步过去的是 Windows 这一端的插件，以及配置文件本身。

#### 换电脑之后

装好 VSCode、登录同一个账号，VSCode 会问你本地和云端怎么处理，想拿到和原来一模一样的配置就选 Replace Local（用云端的覆盖本地）：

- Merge：本地和云端合并
- Replace Local：用云端的覆盖本地
- Merge Manually...：一项一项手动挑

另外，和具体机器绑定的那类设置（跟本机路径、本机硬件有关的）默认不同步，这是 VSCode 有意的设计。

想关掉同步：同一个齿轮菜单里选 Settings Sync: Turn off；想把云端的数据一起删掉，在那一步的选项里勾上就行。

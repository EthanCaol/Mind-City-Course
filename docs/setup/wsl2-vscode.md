...
!!! tip "补充：用命令行做同样的事"
    ...
    - 用 ++win+r++ 再输入 `cmd` 开出来的终端**不是**管理员，执行上面的命令会报「请求的操作需要提升」
...
=== "方式一：开始菜单"
    ...
    打开之后，**推荐点上「固定到"开始"屏幕」**，以后点一下就能用。

=== "方式二：Win + R 运行 cmd"

    按 ++win+r++ 打开「运行」窗口，输入 `cmd`，回车。

    <!-- 待补图：「运行」窗口中输入 cmd -->

    这样开出来的是**命令提示符**（cmd），不是「Windows 终端」—— 界面长得不太一样，但本文里的命令在两边都能正常用。

    - 粘贴命令是 ++ctrl+v++，和 Windows 终端一样
    - 但**这个办法开出来的 cmd 不是管理员**，本文里凡是标了「管理员」的命令都不能在这儿执行，具体见 [2.1 的补充框](#21-启用-windows-功能)

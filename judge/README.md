...
| 推提交记录 | **每两天的凌晨四点半**检查一次，超过两天没推就推 |
| 推送失败 | 不重试；明天四点半会自动再试一次 |

凌晨四点半是选定的：学生在睡觉，机器也闲，不在白天平白多一份网络活动。
改了名单又不想重启服务，或者想立刻备份，都手动触发：
...
推送就永远不会发生。定时刻检查同理：重启不会把计时清零。

推送失败不做专门重试：每天四点半都会检查，那时提交仍未推送、也仍然超过两天，
自然会再试。失败会记进 `journalctl --user -u mind-city-judge`。
